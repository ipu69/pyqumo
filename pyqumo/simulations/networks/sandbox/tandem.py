...
from pyqumo.randoms import Distribution, CountableDistribution
from pyqumo.stats import build_statistics, TimeSizeRecords

from ..contract import GG1TandemResults
from .components import Queue, FiniteFifoQueue, InfiniteFifoQueue, Server
...
def _build_results(records: Records) -> GG1TandemResults:
    results = GG1TandemResults(records.num_stations)
    ...
    for i in range(records.num_stations):
        ...
        results.system_size.append(CountableDistribution(system_size_pmf))
        results.queue_size.append(CountableDistribution(queue_size_pmf))
        results.busy.append(CountableDistribution(server_size_pmf))
        ...
        # Record 'em!
        results.drop_prob.append(p_drop)
        results.delivery_prob.append(p_delivery)
        ...
        results.departures.append(build_statistics(departures))
        results.arrivals.append(build_statistics(arrivals))
        results.response_time.append(build_statistics([
            p.service_finished[i] - p.arrived[i] for p in served_packets]))
        results.wait_time.append(build_statistics([
            p.service_started[i] - p.arrived[i] for p in served_packets]))
        results.delivery_delays.append(build_statistics([
