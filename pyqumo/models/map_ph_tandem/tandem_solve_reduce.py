...
from pyqumo.randoms import MarkovArrival, PhaseType, Erlang, HyperExponential, \
    Exponential
...
def get_map_ph_1_n_complexity(
        arrival_order: int, service_order: int, capacity: int,
        net_size: int) -> int:
    """
    Compute map_ph_tandem network complexity as `W(V(M+2))^N`.
    """
    return arrival_order * (service_order * (capacity + 2))**net_size


@dataclass
class MapPh1NTandemStats:
    skipped: bool
...
def solve_map_ph_1_n_tandem(
        ...
        max_precise_order: int = 8000) -> MapPh1NTandemStats:
    """
    ...
    while sta_index < net_size:
        ...
        if not reduce_departure and arrival.order > max_precise_order:
            return MapPh1NTandemStats(skipped=True, max_inp_order=arrival.order)
