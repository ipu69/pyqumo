...
from pyqumo.cqumo.models.oqnet.model import simulate_gg1_tandem as c_simulate
...
def test_mm1_tandem(
        props: TandemProps,
        simulate: Callable[..., Results]):
    tol = props.tol

    # FIXME: right now functions have different signatures. Adjust manually.

    kwargs = {'max_packets': props.max_packets}
    if simulate == py_simulate:
        # For pythonic simulator, we need to pass num_stations
        # argument, since it allows to specify service times as a single
        # distribution.
        kwargs.update({'num_stations': props.num_stations})
    else:
        # For cython simulator, no `num_stations` parameter exists.
        # It determines the number of servers from `service` vector length.
        # If service is given with a single distribution, duplicate it.
        if not isinstance(props.service, Iterable):
            props.service = [
                props.service.copy() for _ in range(props.num_stations)
            ]

    ret = simulate(props.arrival, props.service, props.queue_capacity, **kwargs)
    ...
    for i in range(props.num_stations):
        ...
        # FIXME: cython simulator doesn't record arrivals stats properly
        if simulate == py_simulate:
            assert_allclose(
